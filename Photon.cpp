...
void Photon::updatePositionPh( double theta_1, double phi_1, Setup* setup ) {
    ...
    if ( setup->checkPosition(x) == true ) {
        ...
        if ( ran > 0.5) { //reflection true TODO >=, <=
            //Here we have to insert the reflection
            ...
            theta_ph_out = acos(proj_z/norm_proj); //I use step length instead of norm projection
...
void Photon::rotateProjections( double theta_1, double phi_1 ) {
    ...
    if (p_id == 22) { //solo di controllo per sicurezza
        ...
        std::cout << "-> Now the step length is      : " << sqrt((proj_x*proj_x + proj_y*proj_y + proj_z*proj_z)) << std::endl;
...
void Photon::reflectionPh(double r) {//input: the radius of the cylinder
    ...
    //The three components of the versor u of the shift;
    double vers_shift_x = proj_x/step_length;
    //std::cout << "vers_shift_x " << vers_shift_x << std::endl;
    double vers_shift_y = proj_y/step_length;
    //std::cout << "vers_shift_y " << vers_shift_y << std::endl;
    double vers_shift_z = proj_z/step_length;
    //std::cout << "vers_shift_z " << vers_shift_z << std::endl;
    
    //Compute the angle between the normal to the plane and the versor of the shift as the internal product between them. The normal to the plan is the radius. Compute the versor of the radius. The coordinates of the radius are those of the intersection point.
    ...
    double theta_0 = (vers_r_x*vers_shift_x) + (vers_r_y*vers_shift_y);
    //std::cout << "theta_0 " << theta_0 << std::endl;
    double phi_0   = (M_PI/2 - theta_0);
    ...
    if ( sqrt(x->getX()*x->getX() + x->getY()*x->getY()) >= r) {//REFLECTION ON THE LATERAL WALL.
        ...
        //Find the componet of the shift w.r. to the plan and it's normal vector.
        double shift_normal = step_length * cos(theta_0);
        ...
        double shift_plan   = step_length * cos(phi_0  ); //è la y
        ...
        double proj_x = shift_normal_new*(-vers_r_x) - shift_plan*vers_r_y;
        double proj_y  = -shift_normal_new*vers_r_y + shift_plan*vers_r_x;
        
        std::cout << "-> New shift projections: (" << proj_x << ", " << proj_y << ", " << proj_z << ")" << std::endl;
        norm_proj =  norm_proj = sqrt((proj_x*proj_x + proj_y*proj_y + proj_z*proj_z));
        std::cout << "-> Shift norm: " << norm_proj << std::endl;
